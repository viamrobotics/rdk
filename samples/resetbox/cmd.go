...
func (a *LinearAxis) SetZeroPosition(ctx context.Context, offset float64) error {
	var errs error
	for _, m := range a.m {
		multierr.AppendInto(&errs, m.SetZeroPosition(ctx, offset))
	}
	return errs
}
...
func (b *ResetBox) home(ctx context.Context) error {
	...
	errs := multierr.Combine(
		...
		<-errPath,
		<-errPath,
		b.gate.SetZeroPosition(ctx, 0),
		b.squeeze.SetZeroPosition(ctx, 0),
		b.elevator.SetZeroPosition(ctx, 0),
		b.hammer.SetZeroPosition(ctx, 0),
	)
...
func (b *ResetBox) hammerTime(ctx context.Context, count int) error {
	...
	err = b.hammer.SetZeroPosition(ctx, 0)
	if err != nil {
		return err
	}
