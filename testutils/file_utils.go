...
func BuildTempModuleWithFirstRun(tb testing.TB, modDir string) string {
	...
	for _, cp := range []copyOp{
		...
		// TODO: This is a horrible hack that attempts to get around a bad path generated by
		// [utils.ResolveFile] in CI. Figure out the actual problem before merging.
		srcPathHACK := strings.ReplaceAll(srcPath, "rdk/rdk", "rdk")

		//nolint:gosec
		src, err := os.Open(srcPathHACK)
		if err != nil {
			tb.Fatal(err)
		}
