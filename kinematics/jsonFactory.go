...
func ParseJSON(jsonData []byte) (*Model, error) {
	...
	if m.Model.Param == "SVA" || m.Model.Param == "" {
	...
	} else if m.Model.Param == "DH" {
		for _, dh := range m.Model.DHParams {

			// Joint part of DH param
			jointID := dh.ID + "_j"
			j := NewJoint([]int{0, 0, 1}, "ccw", dh.Parent)
		...
		return nil, errors.Errorf("unsupported param type: %s, supported params are SVA and DH", m.Model.Param)
	}
