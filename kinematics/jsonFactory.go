...
type AutoGenerated struct {
	Model struct {
		...
		Links        []struct {
			...
			Orientation spatialmath.OrientationVecDegrees `json:"orientation"`
		} `json:"links"`
		Joints []struct {
			ID     string `json:"id"`
...
func ParseJSON(jsonData []byte) (*Model, error) {
	...
	if m.Model.KinParamType == "SVA" || m.Model.KinParamType == "" {
		...
		for _, link := range m.Model.Links {
			...
			ov := &spatialmath.OrientationVec{utils.DegToRad(link.Orientation.Theta), link.Orientation.OX, link.Orientation.OY, link.Orientation.OZ}
		...
		for _, joint := range m.Model.Joints {
			...
			if joint.Type == "revolute" {
				...
				rev := frame.NewRotationalFrame(joint.ID, aa, frame.Limit{joint.Min * math.Pi / 180, joint.Max * math.Pi / 180})
		...
		for _, dh := range m.Model.DHParams {
			...
			j := frame.NewRotationalFrame(jointID, spatialmath.R4AA{RX: 0, RY: 0, RZ: 1}, frame.Limit{dh.Min * math.Pi / 180, dh.Max * math.Pi / 180})
