...
func ParseJSON(jsonData []byte) (*Model, error) {
	...
	for _, link := range m.Model.Links {
		...
		if link.SetOrientation {
			newOV := &kinmath.OrientationVec{link.Orientation.TH, link.Orientation.X, link.Orientation.Y, link.Orientation.Z}
			linkT.t = kinmath.NewQuatTransFromRotation(newOV)
		}
