...
do_mise(){
	if ! test -e $HOME/.local/bin/mise; then
		# This is the default install location but we're being explicit here so this
		# doesn't break if upstream decides to change it.
		curl https://mise.run | MISE_INSTALL_PATH=$HOME/.local/bin/mise sh
	else
		mise self-update -y
	fi
	
	# Could add the entire contents generated by `mise activate <shell>` here but
	# keeping it minimal for now. Users who are more bought into the mise
	# ecosystem probably already have it activated in their shell anyway.
	echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.viamdevrc

	# Turn on mise auto update. 7 days is the default but specify it here so it's
	# easier to find and stays the same even it's changed upstream.
	mise trust -y
