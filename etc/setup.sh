...
do_linux(){
	...
	if apt-get --version > /dev/null 2>&1; then
		...
		# New Fedora/Redhat
		INSTALL_CMD="dnf5 -y install procps-ng curl git which libstdc++-static && dnf5 group install -y c-development development-tools"
...
do_mise(){
	...
	echo 'eval "$(~/.local/bin/mise activate)"' >> ~/.viamdevrc
