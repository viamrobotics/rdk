...
func TestLoginAction(t *testing.T) {
	cCtx, ac, out, errOut := setup(nil, nil, nil, nil, "token", nil)
...
func TestAPIKeyAuth(t *testing.T) {
	_, ac, _, errOut := setup(nil, nil, nil, nil, "apiKey", nil)
	test.That(t, len(errOut.messages), test.ShouldEqual, 0)
...
func TestPrintAccessTokenAction(t *testing.T) {
	...
	cCtx, ac, out, errOut := setup(&inject.AppServiceClient{}, nil, nil, nil, "token", nil)
...
func TestRobotAPIKeyCreateAction(t *testing.T) {
	...
	cCtx, ac, out, _ = setup(asc, nil, nil, nil, "token", flags)
	err = ac.robotAPIKeyCreateAction(cCtx)
	test.That(t, err, test.ShouldNotBeNil)
...
func TestLocationAPIKeyCreateAction(t *testing.T) {
	...
	cCtx, ac, _, _ = setup(asc, nil, nil, nil, "token", flags)
