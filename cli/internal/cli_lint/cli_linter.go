...
func enforceCreateCommandWithTRun(pass *analysis.Pass) (interface{}, error) {
	...
	for _, pkg := range pass.Pkg.Imports() {
		...
		if pkg.Name() == "cli" && !strings.Contains(pkg.Path(), "viam") {
			...
			commandType = types.NewPointer(commandType)
		}
	}
	...
	for _, file := range pass.Files {
		ast.Inspect(file, func(node ast.Node) bool {
			...
			for _, elt := range composite.Elts {
				if ident, isIdent := elt.(*ast.KeyValueExpr); isIdent {
					key := ident.Key.(*ast.Ident)
					...
					if key.Name == "Action" || key.Name == "Before" ||
						key.Name == "After" {
						callExpr, isCallExpr := ident.Value.(*ast.CallExpr)
