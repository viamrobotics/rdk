...
func TestConfigValidation(t *testing.T) {
	...
	t.Run("valid config with sensor", func(t *testing.T) {
		...
		mode, err := defaultslam.RuntimeConfigValidation(cfg, logger)
		test.That(t, err, test.ShouldBeNil)
	...
	t.Run("run test of config with no sensor", func(t *testing.T) {
		cfg := getValidConfig(name1)
		cfg.Sensors = []string{}
		_, err = defaultslam.RuntimeConfigValidation(cfg, logger)
		test.That(t, err, test.ShouldBeNil)
	})
...
func getValidConfig(dataDirectory string) *defaultslam.AttrConfig {
	return &defaultslam.AttrConfig{
