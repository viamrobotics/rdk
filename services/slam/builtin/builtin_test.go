...
func TestBuiltinQuaternion(t *testing.T) {
	t.Run("test successful quaternion from internal server", func(t *testing.T) {
		...
		pose, componentRef, err := builtin.CheckQuaternionFromClientAlgo(poseSucc, componentRefSucc, returnedExtSucc)
		test.That(t, err, test.ShouldBeNil)
	...
	t.Run("test failure due to quaternion not being given", func(t *testing.T) {
		...
		pose, componentRef, err := builtin.CheckQuaternionFromClientAlgo(poseSucc, componentRefSucc, returnedExtFail)
