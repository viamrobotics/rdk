...
func NewBuiltIn(ctx context.Context, deps registry.Dependencies, config config.Service, logger golog.Logger, bufferSLAMProcessLogs bool) (slam.Service, error) {
	...
	port, dataRateMsec, mapRateSec, useLiveData, deleteProcessedData, err := slamConfig.SetOptionalParameters(svcConfig, localhost0, defaultDataRateMsec, defaultMapRateSec, logger)
	if err != nil {
		return nil, err
	}
