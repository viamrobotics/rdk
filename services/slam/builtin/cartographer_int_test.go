...
func integrationtestHelperCartographer(t *testing.T, mode slam.Mode) {
	...
	svc, err := createSLAMService(t, slamConf, "cartographer", logger, true, true)
	test.That(t, err, test.ShouldBeNil)
	...
	testCartographerPosition(t, svc, slamConf.Sensors[0])
	...
	// Create slam service using a real cartographer binary
	svc, err = createSLAMService(t, slamConf, "cartographer", golog.NewTestLogger(t), true, true)
	test.That(t, err, test.ShouldBeNil)
	...
	time.Sleep(time.Second * time.Duration(*slamConf.MapRateSec))
