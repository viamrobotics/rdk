...
func ensurePartsAreEqual(part *config.FrameSystemPart, otherPart *config.FrameSystemPart) error {
	...
	} else if otherOrient == nil {
		return errors.New("orientation not returned for other part")
	} else if !spatialmath.OrientationAlmostEqual(orient, otherOrient) {
		return errors.New("orientations of parts not equal")
	}
	return nil
}
...
func TestClientConfig(t *testing.T) {
	...
	workingFSClient, err := framesystem.NewClient(context.Background(), framesystem.Name.String(), listener1.Addr().String(), logger, rpc.WithInsecure())
	test.That(t, err, test.ShouldBeNil)
	...
	failingFSClient, err := framesystem.NewClient(context.Background(), framesystem.Name.String(), listener2.Addr().String(), logger, rpc.WithInsecure())
	test.That(t, err, test.ShouldBeNil)
	...
	failingFSClient2, err := framesystem.NewClient(context.Background(), framesystem.Name.String(), listener3.Addr().String(), logger, rpc.WithInsecure())
	test.That(t, err, test.ShouldBeNil)
