...
func (svc *Service) initializeOrUpdateCollector(componentName string, attributes componentAttributes, updateCaptureDir bool) (
	*componentMethodMetadata, error) {
	...
	// Set buffer size to defaultCaptureBufferSize if it was not set in the config.
	...
	collector, err := (*collectorConstructor)(res, componentName, interval, attributes.AdditionalParams, targetFile, captureBufferSize, svc.logger)
	if err != nil {
		return nil, err
	}
