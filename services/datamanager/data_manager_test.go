...
func TestWrapWithReconfigurable(t *testing.T) {
	...
	reconfSvc1, err := datamanager.WrapWithReconfigurable(svc, resource.Name{})
	test.That(t, err, test.ShouldBeNil)

	_, err = datamanager.WrapWithReconfigurable(nil, resource.Name{})
	test.That(t, err, test.ShouldBeError, datamanager.NewUnimplementedInterfaceError(nil))

	reconfSvc2, err := datamanager.WrapWithReconfigurable(reconfSvc1, resource.Name{})
	test.That(t, err, test.ShouldBeNil)
...
func TestReconfigurable(t *testing.T) {
	...
	reconfSvc1, err := datamanager.WrapWithReconfigurable(actualSvc1, resource.Name{})
	test.That(t, err, test.ShouldBeNil)
	test.That(t, reconfSvc1, test.ShouldNotBeNil)

	actualSvc2 := &mock{name: testSvcName2}
	reconfSvc2, err := datamanager.WrapWithReconfigurable(actualSvc2, resource.Name{})
	test.That(t, err, test.ShouldBeNil)
