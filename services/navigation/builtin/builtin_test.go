...
func TestSanity(t *testing.T) {
	...
	detection, _ := spatialmath.NewBox(
		spatialmath.NewPose(r3.Vector{4, 8, 10}, &spatialmath.OrientationVectorDegrees{OZ: 1}),
		r3.Vector{5, 7, 2}, "det",
	)

	cameraOrigin := referenceframe.NewPoseInFrame("test_camera", spatialmath.NewZeroPose())
	cameraToWorld, err := fsSvc.TransformPose(ctx, cameraOrigin, "world", nil)
	if err != nil {
	...
	// inverse := spatialmath.PoseInverse(cameraToWorld.Pose())
	// fmt.Println("inverse: ", spatialmath.PoseToProtobuf(inverse))

	new := detection.Transform(cameraToWorld.Pose())
	fmt.Println("new: ", spatialmath.PoseToProtobuf(new.Pose()))

	// baseTheta := -90.
	// p := spatialmath.NewPoseFromOrientation(&spatialmath.OrientationVectorDegrees{OZ: 1, Theta: baseTheta + 90})
	// new = new.Transform(p)
	// fmt.Println("new: ", spatialmath.PoseToProtobuf(new.Pose()))
}

func TestSmth(t *testing.T) {
	p := spatialmath.NewPoseFromOrientation(&spatialmath.OrientationVectorDegrees{OX: 1})
	q := spatialmath.NewPose(r3.Vector{4, 0, 10}, &spatialmath.OrientationVectorDegrees{OZ: 1})
	a := spatialmath.Compose(p, q)
	fmt.Println("a: ", spatialmath.PoseToProtobuf(a))
}
