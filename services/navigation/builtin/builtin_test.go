...
func TestSanity(t *testing.T) {
	...
	cameraToBase, err := fsSvc.TransformPose(ctx, cameraOrigin, "test_base", nil)
	if err != nil {
		// here we make the assumption the camera is coincident with the world
	...
	fmt.Println(" ")
	fmt.Println("cameraToBase: ", spatialmath.PoseToProtobuf(cameraToBase.Pose()))
	new = detecetion.Transform(cameraToBase.Pose())
	fmt.Println("new: ", spatialmath.PoseToProtobuf(new.Pose()))
	new = new.Transform(spatialmath.PoseInverse(motion.SLAMOrientationAdjustment))
	fmt.Println("new: ", spatialmath.PoseToProtobuf(new.Pose()))
}
