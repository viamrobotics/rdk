...
func (m *movementSensorLocalizer) CurrentPosition(ctx context.Context) (*referenceframe.PoseInFrame, error) {
	...
	case properties.CompassHeadingSupported:
		...
		// NOTE: I still don't understand how to think about handedness when it comes to compass heading.
		// Why is it that N=0, E=90, S=180, W=270 is left handed?
		heading := rdkutils.SwapCompasHeadingHandedness(headingLeft)
		// NOTE: I'm very confused why we need to put the heading information in Theta.
		// the only docs I'm aware of for orientation vector are: https://docs.viam.com/internals/orientation-vector/
		// which indicates that Theta describes an angle of rotation about the calculated vector (X: 0, Y: 0, Z: 1) in this case.
		// As a result,  I'm under the impression that Theta describes roll. Why are we putting heading information, describes a direction
		// the base is poining into theta? Why are we not putting that information into OZ, OX & OY so that the vector points in the
		// direction described by the heading?
