...
func TestGetPose(t *testing.T) {
	...
	pose, err := ms.GetPose(context.Background(), arm.Named("gantry1"), "", []*commonpb.Transform{})
	test.That(t, err, test.ShouldBeNil)
	...
	pose, err = ms.GetPose(context.Background(), arm.Named("arm1"), "", []*commonpb.Transform{})
	test.That(t, err, test.ShouldBeNil)
	test.That(t, pose.FrameName(), test.ShouldEqual, referenceframe.World)
	...
	pose, err = ms.GetPose(context.Background(), arm.Named("arm1"), "gantry1", []*commonpb.Transform{})
	test.That(t, err, test.ShouldBeNil)
	...
	pose, err = ms.GetPose(context.Background(), arm.Named("gantry1"), "gantry1", []*commonpb.Transform{})
	test.That(t, err, test.ShouldBeNil)
	test.That(t, pose.FrameName(), test.ShouldEqual, "gantry1")
	...
	pose, err = ms.GetPose(context.Background(), arm.Named("arm1"), "arm1", []*commonpb.Transform{})
	test.That(t, err, test.ShouldBeNil)
