...
func (mr *moveRequest) Plan(ctx context.Context) (motionplan.Plan, error) {
	...
	mr.planRequest.StartState = mpimpl1.NewPlanState(mr.planRequest.StartState.Poses(), startConf)
	...
	return mpimpl1.Replan(ctx, mr.logger, &planRequestCopy, nil, 0)
}
...
func (mr *moveRequest) deviatedFromPlan(ctx context.Context, plan motionplan.Plan) (state.ExecuteResponse, error) {
	...
	errorState, err := mpimpl1.CalculateFrameErrorState(executionState, k, mr.kinematicBase.LocalizationFrame())
	if err != nil {
		return state.ExecuteResponse{}, err
	}
	...
	for visSrvc, cameraNames := range mr.obstacleDetectors {
		for _, camName := range cameraNames {
			...
			mr.logger.CDebugf(ctx, "CheckPlan inputs: \n currentPosition: %v\n currentInputs: %v\n worldstate: %s",
				updatedBaseExecutionState.CurrentPoses()[k.Name()].Pose(),
				updatedBaseExecutionState.CurrentInputs(),
				worldState.String(),
			)
			if err := mpimpl1.CheckPlan(
...
func (mr *moveRequest) augmentBaseExecutionState(
	ctx context.Context,
	baseExecutionState mpimpl1.ExecutionState,
) (mpimpl1.ExecutionState, error) {
	k, err := mr.kinematicBase.Kinematics(ctx)
	if err != nil {
	...
	for idx, currTraj := range existingPlan.Trajectory() {
		...
		if idx == baseExecutionState.Index() {
			...
			kbPose := currFrameSystemPoses[k.Name()]
			trajPose, err := k.Transform(kbTraj)
			if err != nil {
	...
	kinematicBaseCurrentInputs := allCurrentInputsFromBaseExecutionState[k.Name()]
	...
	allCurrentInputsFromBaseExecutionState[k.Name()] = kinematicBaseCurrentInputs
...
func kbOptionsFromCfg(motionCfg *validatedMotionConfiguration, validatedExtra validatedExtra) kinematicbase.Options {
	...
	if validatedExtra.motionProfile != "" {
		kinematicsOptions.PositionOnlyMode = validatedExtra.motionProfile == mpimpl1.PositionOnlyMotionProfile
	}
...
func (ms *builtIn) newMoveOnMapRequest(
	...
	octree, err := pointcloud.ReadPCD(bytes.NewReader(pointCloudData), pointcloud.BasicOctreeType)
	if err != nil {
		return nil, err
	}
...
func (ms *builtIn) createBaseMoveRequest(
	...
	kinematicFrame, err := kb.Kinematics(ctx)
	if err != nil {
		return nil, err
	}
	...
	executionFrame, err := kb.Kinematics(ctx)
	if err != nil {
		return nil, err
	}
	...
	atGoalCheck := func(basePose spatialmath.Pose) bool {
		if valExtra.motionProfile == mpimpl1.PositionOnlyMotionProfile {
	...
	goals := []*mpimpl1.PlanState{mpimpl1.NewPlanState(referenceframe.FrameSystemPoses{kinematicFrame.Name(): goal}, nil)}

	planOpts, err := mpimpl1.NewPlannerOptionsFromExtra(valExtra.extra)
	if err != nil {
		return nil, err
	}
	mr := &moveRequest{
