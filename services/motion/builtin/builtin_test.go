...
func TestCheckPlan(t *testing.T) {
	...
	baseExecutionState, err := mpimpl1.NewExecutionState(
	...
	t.Run("base case - validate plan without obstacles", func(t *testing.T) {
		err = mpimpl1.CheckPlan(wrapperFrame, augmentedBaseExecutionState, nil, mr.localizingFS, math.Inf(1), logger)
		test.That(t, err, test.ShouldBeNil)
	})
	...
	t.Run("obstacles blocking path", func(t *testing.T) {
		...
		err = mpimpl1.CheckPlan(wrapperFrame, augmentedBaseExecutionState, worldState, mr.localizingFS, math.Inf(1), logger)
		test.That(t, err, test.ShouldNotBeNil)
	...
	executionStateWithCamera, err := mpimpl1.NewExecutionState(
	...
	t.Run("obstacles NOT in world frame - no collision - integration test", func(t *testing.T) {
		...
		err = mpimpl1.CheckPlan(wrapperFrame, executionStateWithCamera, worldState, mr.localizingFS, math.Inf(1), logger)
		test.That(t, err, test.ShouldBeNil)
	})
	...
	newExecutionState, err := mpimpl1.NewExecutionState(plan, 2, currentInputs, currentPoses)
	test.That(t, err, test.ShouldBeNil)
	...
	t.Run("checking from partial-plan, ensure success with obstacles - integration test", func(t *testing.T) {
		...
		err = mpimpl1.CheckPlan(wrapperFrame, updatedExecutionState, worldState, mr.localizingFS, math.Inf(1), logger)
		test.That(t, err, test.ShouldBeNil)
	})
...
func TestDoCommand(t *testing.T) {
	...
	t.Run("Extras transmitted correctly", func(t *testing.T) {
		...
		moveReq.Extra = map[string]interface{}{
			"motion_profile": mpimpl1.LinearMotionProfile,
		...
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err, test.ShouldResemble, mpimpl1.NewAlgAndConstraintMismatchErr("rrtstar"))
	})
...
func TestMultiWaypointPlanning(t *testing.T) {
	...
	t.Run("plan through multiple pose waypoints", func(t *testing.T) {
		...
		wp1State := mpimpl1.NewPlanState(referenceframe.FrameSystemPoses{"pieceGripper": waypoint1}, nil)
		wp2State := mpimpl1.NewPlanState(referenceframe.FrameSystemPoses{"pieceGripper": waypoint2}, nil)
	...
	t.Run("plan with pose start state", func(t *testing.T) {
		...
		startState := mpimpl1.NewPlanState(referenceframe.FrameSystemPoses{"pieceGripper": start}, nil)
		wpState := mpimpl1.NewPlanState(referenceframe.FrameSystemPoses{"pieceGripper": waypoint}, nil)
	...
	t.Run("plan through mixed pose and configuration waypoints", func(t *testing.T) {
		...
		wp1State := mpimpl1.NewPlanState(nil, referenceframe.FrameSystemInputs{
	...
	t.Run("plan with custom start state", func(t *testing.T) {
		...
		startState := mpimpl1.NewPlanState(nil, referenceframe.FrameSystemInputs{
	...
	t.Run("plan with explicit goal state configuration", func(t *testing.T) {
		...
		goalState := mpimpl1.NewPlanState(nil, referenceframe.FrameSystemInputs{"pieceArm": referenceframe.FloatsToInputs(goalConfig)})
