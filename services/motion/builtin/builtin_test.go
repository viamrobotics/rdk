...
func TestCheckPlan(t *testing.T) {
	...
	startPose := spatialmath.NewPoseFromPoint(r3.Vector{X: 0, Y: 0, Z: 0})
	...
	t.Run("with a blocking obstacle - ensure failure", func(t *testing.T) {
		obstacle, err := spatialmath.NewBox(
			spatialmath.NewPoseFromPoint(r3.Vector{X: 0, Y: 380, Z: 0}), // Y means forwards from the base's pose at the start of the motion
			r3.Vector{X: 10, Y: 10, Z: 10}, "obstacle",
		)
		test.That(t, err, test.ShouldBeNil)
	...
	cameraOriginFrame, err := referenceframe.NewStaticFrame("camera-origin", spatialmath.NewPoseFromPoint(r3.Vector{X: 0, Y: -30, Z: 0}))
	test.That(t, err, test.ShouldBeNil)
	...
	t.Run("ensure transforms of obstacles works - no collision", func(t *testing.T) {
		// create obstacle
		obstacle, err := spatialmath.NewBox(
			spatialmath.NewPoseFromPoint(r3.Vector{X: -6, Y: 1500, Z: 0}),
			r3.Vector{X: 10, Y: 10, Z: 10}, "obstacle",
		)
		test.That(t, err, test.ShouldBeNil)
		geoms := []spatialmath.Geometry{obstacle}
	...
	t.Run("ensure transforms of obstacles works - collision with camera", func(t *testing.T) {
		// create obstacle
		obstacle, err := spatialmath.NewBox(
			spatialmath.NewPoseFromPoint(r3.Vector{X: 0, Y: 400, Z: 0}),
			r3.Vector{X: 50, Y: 50, Z: 10}, "obstacle",
		)
		test.That(t, err, test.ShouldBeNil)
		geoms := []spatialmath.Geometry{obstacle}
		gifs := []*referenceframe.GeometriesInFrame{referenceframe.NewGeometriesInFrame(cameraFrame.Name(), geoms)}
	...
	t.Run("non nil error state - ensure success", func(t *testing.T) {
		errorState := spatialmath.NewPoseFromPoint(r3.Vector{X: 0, Y: 2600, Z: 0})

		obstacle, err := spatialmath.NewBox(
			spatialmath.NewPoseFromPoint(r3.Vector{X: 0, Y: 150, Z: 0}),
			r3.Vector{X: 10, Y: 10, Z: 1}, "obstacle",
		)
		test.That(t, err, test.ShouldBeNil)
		geoms := []spatialmath.Geometry{obstacle}
		gifs := []*referenceframe.GeometriesInFrame{referenceframe.NewGeometriesInFrame(referenceframe.World, geoms)}
