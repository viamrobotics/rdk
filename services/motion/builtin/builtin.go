...
func (ms *builtIn) BuiltInReconfigure(
	...
	// API's sub before classifying, or a composite movement_sensor/slam/vision/arm falls through to the
	// component bucket as an opaque wrapper. No-op for an ordinary resource. Component subs are grouped
	// by short name and resolved below, since a composite may serve several component APIs under one name.
	...
	for name, subs := range componentsByName {
		...
		// silently pick one. With no kinematic sub, any (there is only one, unless remote-composite
		// siblings) works, since it is not used for kinematics.
