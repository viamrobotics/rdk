...
func init() {
	registry.RegisterLidar(ModelNameClient, &registry.LidarRegistration{Constructor: func(ctx context.Context, r robot.Robot, config config.Component, logger golog.Logger) (lidar.Lidar, error) {
		address := config.Host
		if config.Port != 0 {
