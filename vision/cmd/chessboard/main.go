...
func getEdgesGrayImageFromPath(pEdges *string) *image.Image {
...
func main() {
	pathImg := flag.String("path", "", "path of image to detect chessboard")
	pathEdges := flag.String("pathEdges", "", "path of edges image in which to detect chessboard")
	conf := flag.String("conf", "", "path of configuration for chessboard detection algorithm")
	flag.Parse()
...
func RunChessBoardDetection(pthImg, pthEdges, pthConf string) int {
	...
	file, err := os.Open(filepath.Clean(pthConf))
	if err != nil {
	...
	edgesGray := getEdgesGrayImageFromPath(&pthEdges)
