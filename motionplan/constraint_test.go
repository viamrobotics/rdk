...
func TestCollisionConstraints(t *testing.T) {
	...
	test.That(t, err, test.ShouldBeNil)
	handler := &ConstraintHandler{}
	collisionConstraints, err := createAllCollisionConstraints(sf, fs, worldState, frame.StartPositions(fs), nil, logging.NewTestLogger(t))
	test.That(t, err, test.ShouldBeNil)
...
func BenchmarkCollisionConstraints(b *testing.B) {
	...
	test.That(b, err, test.ShouldBeNil)
	handler := &ConstraintHandler{}
	collisionConstraints, err := createAllCollisionConstraints(sf, fs, worldState, frame.StartPositions(fs), nil, logging.NewLogger("logger"))
	test.That(b, err, test.ShouldBeNil)
	for name, constraint := range collisionConstraints {
		handler.AddStateConstraint(name, constraint)
	}
