...
func EvaluatePlan(plan [][]referenceframe.Input, distFunc ArcMetric) (totalCost float64) {
	if len(plan) < 2 {
		return math.Inf(1)
	}
	for i := 0; i < len(plan)-1; i++ {
		cost := distFunc(&ArcInput{StartConfiguration: plan[i], EndConfiguration: plan[i+1]})
		totalCost += cost
	}
	return totalCost
}
