...
func (pm *planManager) planMultiWaypoint(ctx context.Context, seedPlan Plan) (Plan, error) {
	...
	if pm.request.StartState.Inputs == nil {
	...
	for i, wp := range waypoints {
		...
		if !pm.motionChains.useTPspace && maps == nil {
			if seed != nil {
				...
				wp.startState = &PlanState{Inputs: seed}
			}
...
func (pm *planManager) generateWaypoints(seedPlan Plan, wpi int) ([]atomicWaypoint, error) {
	...
	if wpGoals.FsPoses != nil {
		// Transform goal poses into world frame if needed. This is used for e.g. when a component's goal is given in terms of itself.
		alteredGoals, err := motionChains.translateGoalsToWorldPosition(pm.fs, pm.request.StartState.Inputs, wpGoals)
		if err != nil {
			return nil, err
		}
	...
	for i := 1; i <= numSteps; i++ {
		...
		if wpGoals.FsPoses != nil {
			for frameName, pif := range wpGoals.FsPoses {
				toPose := spatialmath.Interpolate(startPoses[frameName].Pose(), pif.Pose(), by)
				to.FsPoses[frameName] = referenceframe.NewPoseInFrame(pif.Parent(), toPose)
			}
		}
		if wpGoals.Inputs != nil {
			for frameName, inputs := range wpGoals.Inputs {
				...
				toInputs, err := frame.Interpolate(from.Inputs[frameName], inputs, by)
				if err != nil {
					return nil, err
				}
				to.Inputs[frameName] = toInputs
			}
		}
...
func (pm *planManager) planToRRTGoalMap(plan Plan, goal atomicWaypoint) (*rrtMaps, error) {
	...
	if pm.motionChains.useTPspace {
		...
		pathDiff := spatialmath.PoseBetween(oldGoal, goal.goalState.FsPoses[pm.motionChains.ptgFrameName].Pose())
...
func (pm *planManager) planRelativeWaypoint(ctx context.Context, seedPlan Plan) (Plan, error) {
	if pm.request.StartState.FsPoses == nil {
	...
	startPose := pm.request.StartState.FsPoses[pm.motionChains.ptgFrameName].Pose()
	goalPif := pm.request.Goals[0].FsPoses[pm.motionChains.ptgFrameName]
	...
	if pathdebug {
		...
		gifs, err := pm.request.WorldState.ObstaclesInWorldFrame(pm.fs, pm.request.StartState.Inputs)
		if err == nil {
	...
	if pm.planOpts.PositionSeeds > 0 && pm.planOpts.MotionProfile == PositionOnlyMotionProfile {
		err = maps.fillPosOnlyGoal(wp.goalState.FsPoses, pm.planOpts.PositionSeeds)
		if err != nil {
			return nil, err
		}
	} else {
		goalPose := wp.goalState.FsPoses[pm.motionChains.ptgFrameName].Pose()
	...
	startNode := &basicNode{q: zeroInputs, poses: pm.request.StartState.FsPoses}
...
func (pm *planManager) useSubWaypoints(seedPlan Plan, wpi int) bool {
	...
	if pm.request.Goals[wpi].Inputs != nil {
		startState := pm.request.StartState
		if wpi > 0 {
			startState = pm.request.Goals[wpi-1]
		}
		if startState.Inputs == nil {
			return false
		}
	}
