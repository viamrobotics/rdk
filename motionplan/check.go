...
func checkPlanRelative(
	...
	var err error
	toWorld := func(pif *referenceframe.PoseInFrame, inputs referenceframe.FrameConfigurations) (*referenceframe.PoseInFrame, error) {
	...
	for _, segment := range segments {
		...
		for _, interpConfig := range interpolatedConfigurations {
			poseInPathTf, err := sfPlanner.fs.Transform(
				referenceframe.FrameConfigurations{checkFrame.Name(): interpConfig},
