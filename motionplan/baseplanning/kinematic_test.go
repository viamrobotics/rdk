...
func TestDynamicFrameSystemXArm(t *testing.T) {
	...
	transformPoint1, err := fs.Transform(positions.ToLinearInputs(), frame.NewPoseInFrame("xArm6", spatial.NewZeroPose()), frame.World, nil)
	test.That(t, err, test.ShouldBeNil)
...
func TestComplicatedDynamicFrameSystem(t *testing.T) {
	...
	transformPoint1, err := fs.Transform(positions.ToLinearInputs(), frame.NewPoseInFrame("UR5e", spatial.NewZeroPose()), frame.World, nil)
	test.That(t, err, test.ShouldBeNil)
	transformPoint2, err := fs.Transform(positions.ToLinearInputs(), frame.NewPoseInFrame("urCamera", spatial.NewZeroPose()), frame.World, nil)
	test.That(t, err, test.ShouldBeNil)
	transformPoint3, err := fs.Transform(positions.ToLinearInputs(), frame.NewPoseInFrame("xArm6", spatial.NewZeroPose()), frame.World, nil)
	test.That(t, err, test.ShouldBeNil)
	transformPoint4, err := fs.Transform(positions.ToLinearInputs(), frame.NewPoseInFrame("urCamera", spatial.NewZeroPose()), "xArm6", nil)
	test.That(t, err, test.ShouldBeNil)
	...
	positions["UR5e"] = []frame.Input{0, 0, 0, 0, -math.Pi / 2, -math.Pi / 2}
	...
	tf, err := fs.Transform(positions.ToLinearInputs(), frame.NewPoseInFrame("urCamera", targetPoint), frame.World, nil)
	test.That(t, err, test.ShouldBeNil)
	...
	positions["gantryX"] = []frame.Input{worldPointLoc.Point().X - poseXarm.Point().X}
	positions["gantryY"] = []frame.Input{worldPointLoc.Point().Y - poseXarm.Point().Y}

	// Confirm the xArm6 is now at the same location as the point
	newPointXarm, err := fs.Transform(positions.ToLinearInputs(), frame.NewPoseInFrame("xArm6", spatial.NewZeroPose()), frame.World, nil)
	test.That(t, err, test.ShouldBeNil)
	...
	pointCamToXarm, err := fs.Transform(positions.ToLinearInputs(), frame.NewPoseInFrame("urCamera", targetPoint), "xArm6", nil)
	test.That(t, err, test.ShouldBeNil)
