...
func TestNearestNeighbor(t *testing.T) {
	...
	j := &basicNode{q: referenceframe.FrameSystemInputs{"": {0.0}}.ToLinearInputs()}
	...
	for i := 1.0; i < 110.0; i++ {
		iSol := &basicNode{q: referenceframe.FrameSystemInputs{"": {i}}.ToLinearInputs()}
		rrtMap[iSol] = j
		j = iSol
	}

	seed := referenceframe.FrameSystemInputs{"": {23.1}}
	nn := nearestNeighbor(&basicNode{q: seed.ToLinearInputs()}, rrtMap, nodeConfigurationDistanceFunc)
	test.That(t, nn.Q().Get("")[0], test.ShouldAlmostEqual, 23.0)
	...
	nn = nearestNeighbor(&basicNode{q: seed.ToLinearInputs()}, rrtMap, nodeConfigurationDistanceFunc)
	test.That(t, nn.Q().Get("")[0], test.ShouldAlmostEqual, 724.0)
}
