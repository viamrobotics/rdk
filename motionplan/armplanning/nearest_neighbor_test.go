...
func TestNearestNeighbor(t *testing.T) {
	...
	j := &node{inputs: referenceframe.FrameSystemInputs{"": {0.0}}}
	...
	for i := 1.0; i < 110.0; i++ {
		iSol := &node{inputs: referenceframe.FrameSystemInputs{"": {i}}}
		rrtMap[iSol] = j
		j = iSol
	}
	...
	test.That(t, nn.inputs[""][0], test.ShouldAlmostEqual, 23.0)
	...
	nn = nearestNeighbor(&node{inputs: seed}, rrtMap, nodeConfigurationDistanceFunc)
	test.That(t, nn.inputs[""][0], test.ShouldAlmostEqual, 724.0)
}
