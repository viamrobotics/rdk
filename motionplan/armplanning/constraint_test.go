...
func TestConstraintPath(t *testing.T) {
	...
	ciBad := &motionplan.Segment{StartConfiguration: homePos, EndConfiguration: badInterpPos, Frame: modelXarm}
...
func TestLineFollow(t *testing.T) {
	...
	stateCheck := &motionplan.StateFS{Configuration: lastGood.StartConfiguration, FS: fs}
...
func TestCollisionConstraints(t *testing.T) {
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("Test %d", i), func(t *testing.T) {
			err := handler.CheckStateConstraints(&motionplan.State{Configuration: c.input, Frame: model})
...
func BenchmarkCollisionConstraints(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		err = handler.CheckStateConstraints(&motionplan.State{Configuration: frame.FloatsToInputs(rfloats), Frame: model})
		test.That(b, err, test.ShouldBeNil)
	}
}
