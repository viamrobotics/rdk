...
func ConstraintsFromProtobuf(pbConstraint *motionpb.Constraints) *Constraints {
	...
	// TODO: OrientationCloudConstraint has no field in go.viam.com/api's motion.v1.Constraints yet;
	// until it does, it only travels through the Go API and JSON plan requests.
...
func (c *Constraints) ToProtobuf() *motionpb.Constraints {
	...
	// TODO: OrientationCloudConstraint is dropped here (no proto field yet), so a request that relied
	// on it arrives with its orientation unconstrained.
