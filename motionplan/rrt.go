...
func initRRTSolutions(ctx context.Context, mp motionPlanner, seed []referenceframe.Input) *rrtPlanReturn {
	...
	optimalCost := mp.opt().DistanceFunc(&ArcInput{StartConfiguration: seed, EndConfiguration: solutions[0].Q()})
	...
	for _, solution := range solutions {
		if canInterp {
			cost := mp.opt().DistanceFunc(&ArcInput{StartConfiguration: seed, EndConfiguration: solution.Q()})
