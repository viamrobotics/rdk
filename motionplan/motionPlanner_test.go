...
func constrainedXArmMotion() (*planConfig, error) {
	...
	start := &PlanState{Inputs: map[string][]frame.Input{model.Name(): home7}}
	goal := &PlanState{FsPoses: frame.FrameSystemPoses{model.Name(): frame.NewPoseInFrame(frame.World, pos)}}
...
func TestSerializedPlanRequest(t *testing.T) {
	...
	pr := &PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{"xArmVgripper": frame.NewPoseInFrame(frame.World, goal)}}},
		StartState:     &PlanState{Inputs: frame.NewZeroInputs(fs)},
		WorldState:     worldState1,
		Constraints:    constraints,
		PlannerOptions: planOpts,
	}

	jsonData, err := os.ReadFile("plan_request_sample.json")
	test.That(t, err, test.ShouldBeNil)
	...
	test.That(t, geometryIF1.Frame, test.ShouldEqual, geometryIF2.Frame)
...
func TestPlanningWithGripper(t *testing.T) {
	...
	solutionMap, err := PlanMotion(context.Background(), logger, fs, &PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{"gripper": newPose}}},
		StartState:     &PlanState{Inputs: zeroPos},
...
func simple2DMap() (*planConfig, error) {
	...
	goal := &PlanState{FsPoses: frame.FrameSystemPoses{modelName: frame.NewPoseInFrame(frame.World, goalPose)}}
	...
	return &planConfig{
		Start:            &PlanState{Inputs: startInput},
		Goal:             goal,
		FS:               fs,
		Options:          opt,
		ConstraintHander: constraintHandler,
...
func simpleXArmMotion() (*planConfig, error) {
	...
	goal := &PlanState{FsPoses: frame.FrameSystemPoses{
...
func TestArmOOBSolve(t *testing.T) {
	...
	_, err := PlanMotion(context.Background(), logger, fs, &PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{"urCamera": frame.NewPoseInFrame(frame.World, goal1)}}},
		StartState:     &PlanState{Inputs: positions},
		PlannerOptions: NewBasicPlannerOptions(),
	})
...
func TestArmAndGantrySolve(t *testing.T) {
	...
	test.That(t, err, test.ShouldBeNil)
	plan, err := PlanMotion(context.Background(), logger, fs, &PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{"xArmVgripper": frame.NewPoseInFrame(frame.World, goal1)}}},
		StartState:     &PlanState{Inputs: positions},
		PlannerOptions: planOpts,
	})
	test.That(t, err, test.ShouldBeNil)
...
func TestMultiArmSolve(t *testing.T) {
	...
	test.That(t, err, test.ShouldBeNil)
	plan, err := PlanMotion(context.Background(), logger, fs, &PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{"xArmVgripper": frame.NewPoseInFrame("urCamera", goal2)}}},
		StartState:     &PlanState{Inputs: positions},
		PlannerOptions: planOpts,
	})
	test.That(t, err, test.ShouldBeNil)
...
func TestReachOverArm(t *testing.T) {
	...
	test.That(t, err, test.ShouldBeNil)
	plan, err := PlanMotion(context.Background(), logger, fs, &PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{xarm.Name(): goal}}},
		StartState:     &PlanState{Inputs: frame.NewZeroInputs(fs)},
		PlannerOptions: planOpts,
	})
...
func TestPlanMapMotion(t *testing.T) {
	...
	PlanMapMotion := func(
		...
		plan, err := PlanMotion(ctx, logger, fs, &PlanRequest{
			Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{f.Name(): destination}}},
			StartState:     &PlanState{Inputs: seedMap},
			WorldState:     worldState,
			PlannerOptions: NewBasicPlannerOptions(),
		})
		if err != nil {
...
func TestMovementWithGripper(t *testing.T) {
	...
	request := &PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{"xArmVgripper": frame.NewPoseInFrame(frame.World, goal)}}},
		StartState:     &PlanState{Inputs: startConfig},
		PlannerOptions: planOpts,
	}
	solution, err := PlanMotion(context.Background(), logger, fs, request)
	test.That(t, err, test.ShouldBeNil)
	...
	request.Goals = []*PlanState{{FsPoses: frame.FrameSystemPoses{"xArm6": frame.NewPoseInFrame(frame.World, goal)}}}
	_, err = PlanMotion(context.Background(), logger, fs, request)
	test.That(t, err, test.ShouldNotBeNil)
...
func TestReplanValidations(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.msg, func(t *testing.T) {
			...
			test.That(t, err, test.ShouldBeNil)
			_, err = Replan(ctx, logger, baseFS, &PlanRequest{
				Goals: []*PlanState{{FsPoses: frame.FrameSystemPoses{kinematicFrame.Name(): frame.NewPoseInFrame(frame.World, goal)}}},
				StartState: &PlanState{
					Inputs:  frame.NewZeroInputs(baseFS),
					FsPoses: frame.FrameSystemPoses{kinematicFrame.Name(): frame.NewZeroPoseInFrame(frame.World)},
				},
				PlannerOptions: planOpts,
			}, nil, 0)
			if tc.err != nil {
...
func TestReplan(t *testing.T) {
	...
	planRequest := &PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{kinematicFrame.Name(): frame.NewPoseInFrame(frame.World, goal)}}},
		StartState:     &PlanState{Inputs: frame.NewZeroInputs(baseFS)},
		PlannerOptions: NewBasicPlannerOptions(),
	}

	firstplan, err := PlanMotion(ctx, logger, baseFS, planRequest)
	test.That(t, err, test.ShouldBeNil)
	...
	planRequest.Goals = []*PlanState{{FsPoses: frame.FrameSystemPoses{kinematicFrame.Name(): frame.NewPoseInFrame(frame.World, goal)}}}

	// This should easily pass
	newPlan1, err := Replan(ctx, logger, baseFS, planRequest, firstplan, 1.0)
	test.That(t, err, test.ShouldBeNil)
	...
	newPlan2, err := Replan(ctx, logger, baseFS, planRequest, firstplan, 0.1)
...
func TestPtgPosOnlyBidirectional(t *testing.T) {
	...
	bidirectionalPlanRaw, err := PlanMotion(ctx, logger, baseFS, planRequest)
	test.That(t, err, test.ShouldBeNil)
...
func TestValidatePlanRequest(t *testing.T) {
	...
	validGoal := []*PlanState{{
		FsPoses: frame.FrameSystemPoses{"frame1": frame.NewPoseInFrame("frame1", spatialmath.NewZeroPose())},
	}}
	badGoal := []*PlanState{{
		FsPoses: frame.FrameSystemPoses{"frame1": frame.NewPoseInFrame("non-existent", spatialmath.NewZeroPose())},
	}}
	...
	testCases := []testCase{
		...
		},
		{
			name: "nil goal - fail",
			request: PlanRequest{
				StartState: &PlanState{Inputs: map[string][]frame.Input{
					"frame1": {}, "frame2": {{0}},
				}},
			},
	...
	testFn := func(t *testing.T, tc testCase) {
		err := tc.request.validatePlanRequest(fs)
	...
	_, err = PlanMotion(context.Background(), logger, nil, &PlanRequest{})
	test.That(t, err, test.ShouldNotBeNil)
	...
	_, err = PlanMotion(context.Background(), logger, fs, nil)
...
func TestArmGantryCheckPlan(t *testing.T) {
	...
	f := fs.Frame("xArm6")
	planReq := PlanRequest{
		Goals:          []*PlanState{{FsPoses: frame.FrameSystemPoses{"xArm6": frame.NewPoseInFrame(frame.World, goal)}}},
		StartState:     &PlanState{Inputs: frame.NewZeroInputs(fs)},
		PlannerOptions: NewBasicPlannerOptions(),
	}

	plan, err := PlanMotion(context.Background(), logger, fs, &planReq)
	test.That(t, err, test.ShouldBeNil)
