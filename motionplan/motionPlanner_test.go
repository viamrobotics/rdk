...
func constrainedXArmMotion() (*planConfig, error) {
	model, err := frame.ParseModelJSONFile(utils.ResolveFile("services/motion/data/xarm7_kinematics_test.json"), "")
	if err != nil {
		return nil, err
	}
...
func simpleXArmMotion() (*planConfig, error) {
	xarm, err := frame.ParseModelJSONFile(utils.ResolveFile("services/motion/data/xarm7_kinematics_test.json"), "")
	if err != nil {
		return nil, err
	}
...
func TestReachOverArm(t *testing.T) {
	...
	xarm, err := frame.ParseModelJSONFile(utils.ResolveFile("services/motion/data/xarm6_kinematics_test.json"), "")
	test.That(t, err, test.ShouldBeNil)
...
func TestArmConstraintSpecificationSolve(t *testing.T) {
	fs := frame.NewEmptyFrameSystem("")
	x, err := frame.ParseModelJSONFile(utils.ResolveFile("services/motion/data/xarm6_kinematics_test.json"), "")
	test.That(t, err, test.ShouldBeNil)
