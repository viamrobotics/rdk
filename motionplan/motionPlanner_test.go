...
func TestSolverFrameGeometries(t *testing.T) {
	...
	manager := newPlanManager(logger.Sugar(), fs, sf, sf.inputToMap(make([]frame.Input, len(sf.DoF()))), goal, nil, 1, nil)
	test.That(t, err, test.ShouldBeNil)
...
func TestMovementWithGripper(t *testing.T) {
	...
	manager := newPlanManager(logger.Sugar(), fs, sf, zeroPosition, goal, nil, 1, motionConfig)
