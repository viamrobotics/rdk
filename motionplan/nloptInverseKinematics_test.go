...
func TestCreateNloptIKSolver(t *testing.T) {
	...
	pos := spatialmath.NewPoseFromProtobuf(&commonpb.Pose{X: 360, Z: 362})
	seed := referenceframe.FloatsToInputs([]float64{1, 1, 1, 1, 1, 0})

	_, err = solveTest(context.Background(), ik, pos, seed)
	test.That(t, err, test.ShouldBeNil)

	pos = spatialmath.NewPoseFromProtobuf(&commonpb.Pose{X: -46, Y: -23, Z: 372, Theta: utils.RadToDeg(3.92), OX: -0.46, OY: 0.84, OZ: 0.28})
