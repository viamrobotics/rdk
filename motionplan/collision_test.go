...
func TestCheckCollisions(t *testing.T) {
	...
	cg, err := newCollisionGraph(robot, obstacles, nil, true, spatial.CollisionBuffer)
	test.That(t, err, test.ShouldBeNil)
	...
	cg, err = newCollisionGraph(gf.Geometries(), gf.Geometries(), nil, true, spatial.CollisionBuffer)
	test.That(t, err, test.ShouldBeNil)
	test.That(t, len(cg.collisions(spatial.CollisionBuffer)), test.ShouldEqual, 4)
}
