...
func BuildH(imagepts, worldpts []Corner) mat.Vector {
    ...
    if done {
        ...
        k := Hvec.AtVec(len(Sigma)-1)
        
        for i:=0;i<Hvec.Len();i++{       //Lil normalization vibes
...
func GetIntrinsicsFromB(B mat.Vector) {
    ...
    fmt.Printf("v0: %v\n", v0)
    lam := B.AtVec(5) - ((B.AtVec(3) * B.AtVec(3) + v0*(B.AtVec(1) * B.AtVec(3) - B.AtVec(0) * B.AtVec(4)) )/B.AtVec(0)) 
    fmt.Printf("lam: %v\n", lam)
