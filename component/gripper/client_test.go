...
func TestClient(t *testing.T) {
	...
	t.Run("Failing client", func(t *testing.T) {
		...
		cancel()
		_, err = gripper.NewClient(cancelCtx, gripper1, listener1.Addr().String(), rpcclient.DialOptions{Insecure: true}, logger)
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "canceled")
	})

	// working
	gripper1Client, err := gripper.NewClient(context.Background(), gripper1, listener1.Addr().String(), rpcclient.DialOptions{Insecure: true}, logger)
	test.That(t, err, test.ShouldBeNil)
...
func TestClientDialerOption(t *testing.T) {
	...
	client1, err := gripper.NewClient(ctx, gripper1, listener.Addr().String(), rpcclient.DialOptions{Insecure: true}, logger)
	test.That(t, err, test.ShouldBeNil)
	client2, err := gripper.NewClient(ctx, gripper1, listener.Addr().String(), rpcclient.DialOptions{Insecure: true}, logger)
	test.That(t, err, test.ShouldBeNil)
