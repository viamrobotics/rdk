...
func PlanArmLinearMotionWaypoints(ctx context.Context,
	...
	goalPos := spatialmath.NewPoseFromProtobuf(goal)
	...
	for i := 1; i < numSteps; i++ {
		...
		opt := DefaultArmPlannerOptions(from, to, mp.Frame())
		opts = append(opts, opt)
		
		from = to
	}
	goals = append(goals, goalPos)
	opt := DefaultArmPlannerOptions(from, goalPos, mp.Frame())
	opts = append(opts, opt)
	
	// We want to make this of len(goals), not len(0) so that indexing does not fail
	
	results, err := motionplan.PlannerRunner(ctx, mp, goals, seed, opts, 0)
	if err != nil {
		return nil, err
	}
	fmt.Println("results", results)
	
	finalPos, _ := mp.Frame().Transform(results[len(results)-1])
	fmt.Println("finalPos", spatialmath.PoseToProtobuf(finalPos))
...
func DefaultArmPlannerOptions(from, to spatialmath.Pose, f referenceframe.Frame) *motionplan.PlannerOptions{
	opt := motionplan.NewDefaultPlannerOptions()
	...
	collisionConst := motionplan.NewCollisionConstraint(f, map[string]spatialmath.Geometry{}, map[string]spatialmath.Geometry{})
