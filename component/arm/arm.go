...
func Plan(
	...
	destination := referenceframe.NewPoseInFrame(a.ModelFrame().Name() + "_offset", spatialmath.NewPoseFromProtobuf(dst))
	
	return motionplan.PlanRobotMotion(ctx, destination, a.ModelFrame(), r, fs, worldState, DefaultArmPlannerOptions)
}
