...
// A MinimalArm represents a physical robotic arm that exists in three-dimensional space.
type MinimalArm interface {
...
// Arm represents a fully implemented Arm.
type Arm interface {
	generic.Generic
	MinimalArm
}
...
func FromRobot(r robot.Robot, name string) (Arm, error) {
	...
	if full, ok := res.(Arm); ok {
		return full, nil
	}
	if part, ok := res.(MinimalArm); ok {
		return &reconfigurableArm{actual: part}, nil
	}
...
func GoToWaypoints(ctx context.Context, a MinimalArm, waypoints [][]referenceframe.Input) error {
