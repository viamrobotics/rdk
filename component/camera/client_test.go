...
func TestClient(t *testing.T) {
	...
	t.Run("Failing client", func(t *testing.T) {
		...
		cancel()
		_, err = camera.NewClient(cancelCtx, camera1, listener1.Addr().String(), logger)
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "canceled")
	})

	t.Run("camera client 1", func(t *testing.T) {
		camera1Client, err := camera.NewClient(context.Background(), camera1, listener1.Addr().String(), logger)
		test.That(t, err, test.ShouldBeNil)
...
func TestClientDialerOption(t *testing.T) {
	...
	client1, err := camera.NewClient(ctx, camera1, listener.Addr().String(), logger)
	test.That(t, err, test.ShouldBeNil)
	test.That(t, td.NewConnections, test.ShouldEqual, 3)
	client2, err := camera.NewClient(ctx, camera1, listener.Addr().String(), logger)
	test.That(t, err, test.ShouldBeNil)
	test.That(t, td.NewConnections, test.ShouldEqual, 3)
