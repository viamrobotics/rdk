...
func _readNext(r io.Reader) (int64, error) {
	...
	return 0, errtrace.Wrap(errors.Wrapf(err, "got %d bytes", x))
}
...
func ParseRawDepthMap(fn string) (*DepthMap, error) {
	...
	return errtrace.Wrap2(ReadDepthMap(bufio.NewReader(f)))
}
...
func ReadDepthMap(r io.Reader) (*DepthMap, error) {
	...
	case MagicNumIntVersionX: // magic number for VERSIONX
		return errtrace.Wrap2(readDepthMapVersionX(r))
	case MagicNumIntViamTypeLittleEndian: // magic number for ViamCustomType PAMHTPED (LittleEndian DEPTHMAP)
		return errtrace.Wrap2(readDepthMapViam(r))
	default:
		return errtrace.Wrap2(readDepthMapRaw(r, firstBytes))
	}
}
...
func readDepthMapRaw(ff io.Reader, firstBytes int64) (*DepthMap, error) {
	...
	return errtrace.Wrap2(setRawDepthMapValues(f, &dm))
}
...
func readDepthMapViam(ff io.Reader) (*DepthMap, error) {
	...
	_, err := f.Read(data)
	if err != nil {
		return nil, errtrace.Wrap(errors.Wrapf(err, "could not read vnd.viam.dep width"))
	}
	...
	_, err = f.Read(data)
	if err != nil {
		return nil, errtrace.Wrap(errors.Wrapf(err, "could not read vnd.viam.dep height"))
	}
	...
	if err != nil {
		return nil, errtrace.Wrap(errors.Wrapf(err, "could not read vnd.viam.dep data slice"))
	}
	dm.data = datSlice
	return dm, nil
}
...
func readDepthMapVersionX(rr io.Reader) (*DepthMap, error) {
	...
	if bytesPerPixelString != "2" {
		return nil, errtrace.Wrap(errors.Errorf("i only know how to handle 2 bytes per pixel in new format, not %s", bytesPerPixelString))
	}
	...
	if dm.width <= 0 || dm.width >= 100000 || dm.height <= 0 || dm.height >= 100000 {
		return nil, errtrace.Wrap(errors.Errorf("bad width or height for depth map %v %v", dm.width, dm.height))
	}
	...
	for y := 0; y < dm.height; y++ {
		for x := 0; x < dm.width; x++ {
			...
			if n != 2 || err != nil {
				return nil, errtrace.Wrap(errors.Wrapf(err, "didn't read 2 bytes, got: %d x,y: %d,%x", n, x, y))
			}
...
func WriteRawDepthMapToFile(dm image.Image, fn string) (err error) {
	...
	if filepath.Ext(fn) == ".gz" {
		...
		out = gout
		defer func() {
			err = errtrace.Wrap(multierr.Combine(err, gout.Close()))
		}()
	}
...
func WriteRawDepthMapTo(img image.Image, out io.Writer) (int64, error) {
	...
	totalN += int64(n)
	if err != nil {
		return totalN, errtrace.Wrap(err)
	}
	...
	for x := 0; x < width; x++ {
		for y := 0; y < height; y++ {
			...
			default:
				return totalN, errtrace.Wrap(errors.Errorf("cannot convert image type %T to a raw depth format", dm))
			}
			n, err = out.Write(buf)
			totalN += int64(n)
			if err != nil {
...
func WriteViamDepthMapTo(img image.Image, out io.Writer) (int64, error) {
	if lazy, ok := img.(*LazyEncodedImage); ok {
		...
		if err != nil {
			return 0, errtrace.Wrap(errors.Errorf("could not decode LazyEncodedImage to a depth image: %v", err))
		}
	...
	default:
		return totalN, errtrace.Wrap(errors.Errorf("cannot convert image type %T to image/vnd.viam.dep depth format", dm))
	}
	return totalN, nil
}
