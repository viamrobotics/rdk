...
func TestNewWatcherCloud(t *testing.T) {
	...
	appConn, err := grpc.NewAppConn(context.Background(), confToReturn.Cloud.AppAddress, confToReturn.Cloud.ID, confToReturn.Cloud.Secret, confToReturn.Cloud.ID, logger)
	test.That(t, err, test.ShouldBeNil)
	defer appConn.Close()
