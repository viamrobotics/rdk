...
func TestNetAppenderWaitForQueueEmpty(t *testing.T) {
	t.Run("empty queue returns immediately", func(t *testing.T) {
		na := quickFakeAppender(t)
		ctx := context.Background()
		err := na.WaitForQueueEmpty(ctx, 10*time.Millisecond)
		test.That(t, err, test.ShouldBeNil)
	})

	t.Run("queue empties during wait", func(t *testing.T) {
		na := quickFakeAppender(t)
		na.toLog = append(na.toLog, &commonpb.LogEntry{}, &commonpb.LogEntry{})

		// Simulate queue draining in background
		go func() {
			time.Sleep(50 * time.Millisecond)
			na.toLogMutex.Lock()
			na.toLog = nil
			na.toLogMutex.Unlock()
		}()
