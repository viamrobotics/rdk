...
func TestLoggingNeverDedup(t *testing.T) {
	// Create a logger object that will write to the `notStdout` buffer. Explicitly
	// set DeduplicateLogs to true on the registry for the logger.
	registry := newRegistry()
	registry.DeduplicateLogs.Store(true)
	...
	// Assert that loggers whose names end with an excluded suffix do not have their logs
	// deduplicated even though log deduplication is enabled on the registry.
	for neverDedupLoggerNameSuffix := range neverDedupLoggerNameSuffixes {
		loggerName := fmt.Sprintf("foo%s", neverDedupLoggerNameSuffix)
		notStdout := &bytes.Buffer{}
		logger := &impl{
			name:                     loggerName,
			level:                    NewAtomicLevelAt(DEBUG),
		...
		for range 4 {
			...
			assertLogMatches(t, notStdout,
				fmt.Sprintf(`2023-10-30T13:19:45.806Z	INFO	%s	logging/impl_test.go:132	identical message`, loggerName))
		}
	}
}
