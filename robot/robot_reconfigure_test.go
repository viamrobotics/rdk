...
func TestRobotReconfigure(t *testing.T) {
	readConfigFromFile := func(t *testing.T, filePath string) *api.Config {
		conf, err := api.ReadConfig(filePath)
	...
	t.Run("no diff", func(t *testing.T) {
		...
		conf1 := readConfigFromFile(t, "data/diff_config_1.json")
		...
		test.That(t, utils.NewStringSet(robot.LidarDeviceNames()...), test.ShouldBeEmpty)
	...
	t.Run("additive diff", func(t *testing.T) {
		logger := golog.NewTestLogger(t)
		emptyConf := readConfigFromFile(t, "data/diff_config_empty.json")
		conf1 := readConfigFromFile(t, "data/diff_config_1.json")
	...
	t.Run("modificative diff", func(t *testing.T) {
		logger := golog.NewTestLogger(t)
		conf1 := readConfigFromFile(t, "data/diff_config_1.json")
		conf2 := readConfigFromFile(t, "data/diff_config_2.json")
		robot, err := NewRobot(context.Background(), conf1, logger)
		test.That(t, err, test.ShouldBeNil)
		defer func() {
			test.That(t, robot.Close(), test.ShouldBeNil)
	...
	t.Run("mixed diff", func(t *testing.T) {
		logger := golog.NewTestLogger(t)
		conf1 := readConfigFromFile(t, "data/diff_config_1.json")
		conf3 := readConfigFromFile(t, "data/diff_config_3.json")
		robot, err := NewRobot(context.Background(), conf1, logger)
		test.That(t, err, test.ShouldBeNil)
		defer func() {
			test.That(t, robot.Close(), test.ShouldBeNil)
	...
	t.Run("rollback", func(t *testing.T) {
		...
		logger := golog.NewTestLogger(t)
		conf1 := readConfigFromFile(t, "data/diff_config_1.json")
		conf3 := readConfigFromFile(t, "data/diff_config_3_bad.json")
		robot, err := NewRobot(context.Background(), conf1, logger)
		test.That(t, err, test.ShouldBeNil)
		defer func() {
			test.That(t, robot.Close(), test.ShouldBeNil)
