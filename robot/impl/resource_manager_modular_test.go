...
func TestModularResourcesUntrutedEnv(t *testing.T) {
	logger := golog.NewTestLogger(t)
	ctx := context.Background()
	...
	res := newResourceManager(resourceManagerOptions{untrustedEnv: true}, logger)
	mod := &dummyModMan{}
	r := &localRobot{
		manager: res,
		logger:  logger,
	...
	t.Run("process component", func(t *testing.T) {
		...
		mod.iface = &dummyRes{}
		// Add a modular component
		_, err = res.processComponent(ctx, compName, cfg, nil, r)
		test.That(t, err, test.ShouldEqual, errModularResourcesDisabled)

		var old interface{}
		// Reconfigure a modular component
		_, err = res.processComponent(ctx, compName, cfg2, old, r)
		test.That(t, err, test.ShouldEqual, errModularResourcesDisabled)
	})
	...
	t.Run("process service", func(t *testing.T) {
		...
		// Add a modular service
		_, err = res.processService(ctx, cfg, nil, r)
		test.That(t, err, test.ShouldEqual, errModularResourcesDisabled)

		var old interface{}

		// Reconfigure a modular service
		_, err = res.processService(ctx, cfg2, old, r)
		test.That(t, err, test.ShouldEqual, errModularResourcesDisabled)
	})
}
