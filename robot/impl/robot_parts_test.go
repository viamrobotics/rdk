...
func TestPartsAdd(t *testing.T) {
	logger := golog.NewTestLogger(t)
	parts := newRobotParts(false, false, false, nil, logger)
...
func TestPartsNewComponent(t *testing.T) {
	...
	robotForRemote.parts = newRobotParts(false, false, false, nil, logger)
...
func TestPartsMergeAdd(t *testing.T) {
	...
	result, err := parts.MergeAdd(newRobotParts(false, false, false, nil, logger))
	test.That(t, err, test.ShouldBeNil)
	checkSame(parts)

	emptyParts := newRobotParts(false, false, false, nil, logger)
...
func TestPartsMergeModify(t *testing.T) {
	...
	result, err := parts.MergeModify(context.Background(), newRobotParts(false, false, false, nil, logger), &config.Diff{})
	test.That(t, err, test.ShouldBeNil)
	checkSame(parts)
	...
	replacementParts := newRobotParts(false, false, false, nil, logger)
	robotForRemote := &localRobot{parts: newRobotParts(false, false, false, nil, logger), logger: logger}
...
func TestPartsMergeRemove(t *testing.T) {
	...
	parts.MergeRemove(newRobotParts(false, false, false, nil, logger))
	checkSame(parts)
