...
func (p *proxyGPS) Location(ctx context.Context) (lat float64, long float64, err error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.actual.Location(ctx)
}

func (p *proxyGPS) Altitude(ctx context.Context) (alt float64, err error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.actual.Altitude(ctx)
}

func (p *proxyGPS) Speed(ctx context.Context) (kph float64, err error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.actual.Speed(ctx)
}

func (p *proxyGPS) Satellites(ctx context.Context) (active, total int, err error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.actual.Satellites(ctx)
}

func (p *proxyGPS) Accuracy(ctx context.Context) (horizontal, vertical float64, err error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.actual.Accuracy(ctx)
}

func (p *proxyGPS) Valid(ctx context.Context) (valid bool, err error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
