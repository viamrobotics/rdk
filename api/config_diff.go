...
// A ConfigDiff is the difference between two configs, left and right
...
// from left to right.
func DiffConfigs(left, right *Config) (*ConfigDiff, error) {
	prettyDiff, err := prettyConfigDiff(left, right)
	if err != nil {
		return nil, err
	}
...
func prettyConfigDiff(left, right *Config) (string, error) {
...
func (diff *ConfigDiff) String() string {
	return diff.prettyDiff
}

func diffRemotes(left, right []RemoteConfig, diff *ConfigDiff) bool {
	leftIndex := make(map[string]int)
	leftM := make(map[string]RemoteConfig)
...
func diffRemote(left, right RemoteConfig, diff *ConfigDiff) bool {
...
func diffBoards(left, right []board.Config, diff *ConfigDiff) bool {
	leftIndex := make(map[string]int)
...
func diffBoard(left, right board.Config, diff *ConfigDiff) bool {
	if reflect.DeepEqual(left, right) {
		return false
	}
...
func diffComponents(left, right []ComponentConfig, diff *ConfigDiff) bool {
	leftIndex := make(map[string]int)
	leftM := make(map[string]ComponentConfig)
	for idx, l := range left {
		leftM[l.Name] = l
		leftIndex[l.Name] = idx
	}
...
func diffComponent(left, right ComponentConfig, diff *ConfigDiff) bool {
	if reflect.DeepEqual(left, right) {
		return false
	}
...
func diffProcesses(left, right []rexec.ProcessConfig, diff *ConfigDiff) bool {
	leftIndex := make(map[string]int)
...
func diffProcess(left, right rexec.ProcessConfig, diff *ConfigDiff) bool {
	if reflect.DeepEqual(left, right) {
		return false
	}
