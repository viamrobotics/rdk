...
LRESULT CMainFrame::OnDestroy(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
    if (support_motor_ctrl) {
        LidarMgr::GetInstance().lidar_drv->setMotorPWM(0);
    }
...
void    CMainFrame::onRefreshScanData()
{
    ...
    {
		float frequency = 0;
		lidar_drv->getFrequency(inExpressMode, cnt, frequency);
		m_scanview.setScanData(nodes, cnt, frequency, inExpressMode);
    }
}
    ...
    case WORKING_MODE_SCAN:
        {
            ...
            if (support_motor_ctrl) {
                LidarMgr::GetInstance().lidar_drv->setMotorPWM(DEFAULT_MOTOR_PWM);
                Sleep(500);
            }
