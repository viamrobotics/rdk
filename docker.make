...
BUILD_CMD = docker buildx build --pull $(BUILD_PUSH) --force-rm --no-cache --build-arg MAIN_TAG=$(MAIN_TAG) --build-arg BASE_TAG=$(BUILD_TAG) --platform $(PLATFORM) -f $(BUILD_FILE) -t '$(MAIN_TAG):$(BUILD_TAG)-cache' .
BUILD_PUSH = --load
...
canon-cache-amd64-ci: PLATFORM ?= linux/$(BUILD_TAG)
...
canon-cache-arm64-ci: PLATFORM ?= linux/$(BUILD_TAG)
...
antique-cache-amd64: PLATFORM ?= linux/$(BUILD_TAG)
...
antique-cache-arm64: PLATFORM ?= linux/$(BUILD_TAG)
...
antique-cache-amd64-ci: PLATFORM ?= linux/$(BUILD_TAG)
...
antique-cache-arm64-ci: PLATFORM ?= linux/$(BUILD_TAG)
