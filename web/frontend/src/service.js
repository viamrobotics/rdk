...
const connect = async (authEntity = savedAuthEntity, creds = savedCreds) => {
  ...
  const impliedURL = `${location.protocol}//${location.hostname}${
  ...
  if (webrtcEnabled) {
    ...
    const webRTCConn = await dialWebRTC(
      webrtcSignalingAddress || impliedURL,
      webrtcHost,
      opts
    );
    ...
    webRTCConn.peerConnection.ontrack = (event) => {
      ...
      if (
        streamContainer &&
        streamContainer.querySelectorAll('video').length > 0
      ) {
      ...
      const streamPreviewContainer = document.querySelector(
        `#stream-preview-${streamName}`
      );
      if (
        streamPreviewContainer &&
        streamPreviewContainer.querySelectorAll('video').length > 0
      ) {
  ...
  window.streamService = new StreamServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.robotService = new RobotServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  // TODO(RSDK-144): these should be created as needed
  window.armService = new ArmServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.baseService = new BaseServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.boardService = new BoardServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.cameraService = new CameraServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.gantryService = new GantryServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.gripperService = new GripperServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.imuService = new IMUServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.inputControllerService = new InputControllerServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.motorService = new MotorServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.navigationService = new NavigationServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.motionService = new MotionServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.visionService = new VisionServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.sensorsService = new SensorsServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.servoService = new ServoServiceClient(webrtcHost, {
    transport: transportFactory,
  });
  window.slamService = new SLAMServiceClient(webrtcHost, {
    transport: transportFactory,
  });
};
