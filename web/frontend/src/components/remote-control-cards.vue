...
if (props.webrtcAdditionalICEServers) {
    rtcConfig.iceServers = [...rtcConfig.iceServers, ...props.webrtcAdditionalICEServers];
}
const impliedURL = props.host;

const client = new Client(impliedURL, {
    enabled: props.webrtcEnabled,
    host: props.host,
    signalingAddress: props.webrtcSignalingAddress,
    rtcConfig,
});
...
const initConnect = () => {
    if (props.supportedAuthTypes.length === 0) {
...
<template>
    <div id="pre-app">
        <div id="connecting" class="border-greendark hidden border-l-4 bg-gray-100 px-4 py-3">
        ...
        <template v-for="authType in supportedAuthTypes" :key="authType">
            <div class="px-4 py-3">
                <span>{{ authType }}: </span>
                <div class="w-96">
                    <input v-model="password" :disabled="disableAuthElements" class="
            ...
            " type="password" autocomplete="off" @keyup.enter="doLogin(authType)">
                    <v-button :disabled="disableAuthElements" label="Login"
    ...
    <div class="flex flex-col gap-4 p-3">
        <div v-if="errorMessage" class="border-l-4 border-red-500 bg-gray-100 px-4 py-3">
            {{ errorMessage }}
        </div>

        <!-- ******* BASE *******  -->
        <Base v-for="base in filterResources(resources, 'rdk', 'component', 'base')" :key="base.name" :name="base.name"
            :client="client" :resources="resources" />

        <!-- ******* GANTRY *******  -->
        <Gantry v-for="gantry in filterRdkComponentsWithStatus(resources, status, 'gantry')" :key="gantry.name"
            :name="gantry.name" :client="client"
            :status="(resourceStatusByName(gantry) as unknown as ReturnType<typeof fixGantryStatus>)" />

        <!-- ******* MovementSensor *******  -->
        <MovementSensor v-for="sensor in filterResources(resources, 'rdk', 'component', 'movement_sensor')"
            :key="sensor.name" :name="sensor.name" :client="client" />

        <!-- ******* ARM *******  -->
        <Arm v-for="arm in filterResources(resources, 'rdk', 'component', 'arm')" :key="arm.name" :name="arm.name"
            :client="client" :status="(resourceStatusByName(arm) as any)"
            :raw-status="(rawResourceStatusByName(arm) as any)" />

        <!-- ******* GRIPPER *******  -->
        <Gripper v-for="gripper in filterResources(resources, 'rdk', 'component', 'gripper')" :key="gripper.name"
            :name="gripper.name" :client="client" />

        <!-- ******* SERVO *******  -->
        <ServoComponent v-for="servo in filterRdkComponentsWithStatus(resources, status, 'servo')" :key="servo.name"
            :name="servo.name" :client="client" :status="(resourceStatusByName(servo) as any)"
            :raw-status="(rawResourceStatusByName(servo) as any)" />

        <!-- ******* MOTOR *******  -->
        <Motor v-for="motor in filterRdkComponentsWithStatus(resources, status, 'motor')" :key="motor.name"
            :name="motor.name" :client="client" :status="(resourceStatusByName(motor) as any)" />

        <!-- ******* INPUT VIEW *******  -->
        <InputController v-for="controller in filteredInputControllerList()" :key="controller.name"
            :name="controller.name" :status="(resourceStatusByName(controller) as any)" class="input" />

        <!-- ******* WEB CONTROLS *******  -->
        <Gamepad v-for="gamepad in filteredWebGamepads()" :key="gamepad.name" :name="gamepad.name" :client="client" />

        <!-- ******* BOARD *******  -->
        <Board v-for="board in filterRdkComponentsWithStatus(resources, status, 'board')" :key="board.name"
            :name="board.name" :client="client" :status="(resourceStatusByName(board) as any)" />

        <!-- ******* CAMERAS *******  -->
        <CameraView v-for="camera in filterResources(resources, 'rdk', 'component', 'camera')" :key="camera.name"
            :camera-name="camera.name" :client="client" :resources="resources"
            @selected-camera-view="t => { viewCameraFrame(camera.name, t) }" @clear-interval="clearFrameInterval" />

        <!-- ******* NAVIGATION ******* -->
        <Navigation v-for="nav in filterResources(resources, 'rdk', 'service', 'navigation')" :key="nav.name"
            :resources="resources" :name="nav.name" :client="client" />

        <!-- ******* SENSORS ******* -->
        <Sensors v-if="nonEmpty(sensorNames)"
            :name="filterNonRemoteResources(resources, 'rdk', 'service', 'sensors')[0]!.name" :client="client"
            :sensor-names="sensorNames" />

        <!-- ******* AUDIO INPUTS *******  -->
        <AudioInput v-for="audioInput in filterResources(resources, 'rdk', 'component', 'audio_input')"
            :key="audioInput.name" :name="audioInput.name" :client="client" />

        <!-- ******* SLAM *******  -->
        <Slam v-for="slam in filterResources(resources, 'rdk', 'service', 'slam')" :key="slam.name" :name="slam.name"
            :client="client" :resources="resources" />

        <!-- ******* DO ******* -->
        <DoCommand v-if="connectedOnce" :client="client" :resources="filterComponentsWithNames(resources)" />

        <!-- ******* OPERATIONS AND SESSIONS ******* -->
        <OperationsSessions v-if="connectedOnce" :client="client" :operations="currentOps" :sessions="currentSessions"
            :sessions-supported="sessionsSupported" :connection-manager="appConnectionManager" />
    </div>
</template>
