...
func init() {
	registry.RegisterSensor(compass.Type, ModelNameClient, registry.SensorRegistration{Constructor: func(ctx context.Context, r robot.Robot, config config.Component, logger golog.Logger) (sensor.Sensor, error) {
		address := config.Host
		if config.Port != 0 {
