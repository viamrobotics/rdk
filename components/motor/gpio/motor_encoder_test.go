...
func TestMotorEncoderDual(t *testing.T) {
	logger := golog.NewTestLogger(t)
	undo := SetRPMSleepDebug(1, false)
	defer undo()

	type testHarness struct {
		Encoder   *encoder.DualEncoder
	...
	setup := func(t *testing.T) testHarness {
		...
		encoder := &encoder.DualEncoder{A: encoderA, B: encoderB, CancelCtx: context.Background()}
...
func TestWrapMotorWithEncoder(t *testing.T) {
	...
	t.Run("wrap motor with dual-wire encoder", func(t *testing.T) {
		b, err := fakeboard.NewBoard(context.Background(), config.Component{ConvertedAttributes: &board.Config{}}, rlog.Logger)
		test.That(t, err, test.ShouldBeNil)
		fakeMotor := &fakemotor.Motor{}
		b.Digitals["a"] = &board.BasicDigitalInterrupt{}
		b.Digitals["b"] = &board.BasicDigitalInterrupt{}
		e := &encoder.DualEncoder{A: b.Digitals["a"], B: b.Digitals["b"], CancelCtx: context.Background()}
		e.Start(context.Background())
