...
func TestClient(t *testing.T) {
	...
	failingMotor.PropertiesFunc = func(ctx context.Context, extra map[string]interface{}) (motor.Properties, error) {
		return motor.Properties{}, errtrace.Wrap(errPropertiesNotFound)
	}
	...
	test.That(t, resourceAPI.RegisterRPCService(context.Background(), rpcServer, motorSvc, logger), test.ShouldBeNil)
	...
	t.Run("client tests for working motor", func(t *testing.T) {
		...
		statusResult, err := workingMotorClient.Status(context.Background())
		test.That(t, err, test.ShouldBeNil)
