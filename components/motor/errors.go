...
func NewResetZeroPositionUnsupportedError(motorName string) error {
	return errtrace.Wrap(errors.Errorf("motor with name %s does not support ResetZeroPosition", motorName))
}
...
func NewPropertyUnsupportedError(prop Properties, motorName string) error {
	return errtrace.Wrap(errors.Errorf("motor named %s has wrong support for property %#v", motorName, prop))
}
...
func NewZeroRPMError() error {
	return errtrace.Wrap(errors.New("Cannot move motor at an RPM that is nearly 0"))
}

// NewZeroRevsError returns an error representing a request to move a motor for 0 revolutions.
func NewZeroRevsError() error {
	return errtrace.Wrap(errors.New("Cannot move motor for 0 revolutions"))
}
...
func NewGoToUnsupportedError(motorName string) error {
	return errtrace.Wrap(errors.Errorf("motor with name %s does not support GoTo", motorName))
}
...
func NewControlParametersUnimplementedError() error {
	return errtrace.Wrap(errors.New("control parameters must be configured to setup a motor with controls"))
}
...
func NewSetRPMUnsupportedError(motorName string) error {
	return errtrace.Wrap(errors.Errorf("motor named %s does not support SetRPM", motorName))
}
