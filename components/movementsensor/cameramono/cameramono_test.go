...
func TestFunctions(t *testing.T) {
	...
	acc, err := tCo.Accuracy(tCo.cancelCtx, types.ZeroExtraParams())
	test.That(t, acc, test.ShouldResemble, map[string]float32{})
	test.That(t, err,
		test.ShouldResemble,
		movementsensor.ErrMethodUnimplementedAccuracy)

	ch, err := tCo.CompassHeading(tCo.cancelCtx, types.ZeroExtraParams())
