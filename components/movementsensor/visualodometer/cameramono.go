...
func newCameraMono(
	...
	co.backgroundWorker(gostream.NewEmbeddedVideoStream(cam), logger)
	return co, nil
}

func (co *cameramono) backgroundWorker(stream gostream.VideoStream, logger golog.Logger) {
	defer func() {
	...
	utils.ManagedGo(func() {
		for {
			...
			motion, err := odometry.EstimateMotionFrom2Frames(startImage, endImage, nil, logger, false)
			...
			if err != nil {
				...
				case co.output <- motion:
					co.trackedPos.Add(translationToR3(motion)) // most drift occurs here due to underlying algorithm
					co.trackedOrient.RotationMatrix().MatMul(*rotMat)

					// we might want outputs of linear velocity and angular rate to live in the algorithm itself,
					// I can only see point and orientation outputs from motion#D now
...
func (co *cameramono) GetProperties(ctx context.Context) (*movementsensor.Properties, error) {
	return &movementsensor.Properties{
		PositionSupported:        true, // ask Bijan + Khari about the positions.
