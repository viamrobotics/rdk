...
func TestFromDependencies(t *testing.T) {
	...
	result, _, err := s.Position(context.Background(), types.ZeroExtraParams())
	test.That(t, err, test.ShouldBeNil)
...
func TestReconfigurableMovementSensor(t *testing.T) {
	...
	result, _, err := reconfMovementSensor1.(movementsensor.MovementSensor).Position(context.Background(), types.ZeroExtraParams())
	test.That(t, err, test.ShouldBeNil)
	test.That(t, result, test.ShouldResemble, loc)
	test.That(t, actualMovementSensor1.positionCount, test.ShouldEqual, 0)
...
func TestPosition(t *testing.T) {
	...
	test.That(t, actualMovementSensor1.positionCount, test.ShouldEqual, 0)
	loc1, _, err := reconfMovementSensor1.(movementsensor.MovementSensor).Position(context.Background(), types.ZeroExtraParams())
	test.That(t, err, test.ShouldBeNil)
...
func TestLinearVelocity(t *testing.T) {
	...
	speed1, err := reconfMovementSensor1.(movementsensor.MovementSensor).LinearVelocity(context.Background(), types.ZeroExtraParams())
	test.That(t, err, test.ShouldBeNil)
...
func TestReadings(t *testing.T) {
	...
	readings1, err := movementsensor.Readings(context.Background(), actualMovementSensor1, types.ZeroExtraParams())
...
func (m *mock) Position(ctx context.Context, extra types.ExtraParams) (*geo.Point, float64, error) {
	m.positionCount++
	return loc, alt, nil
}

func (m *mock) LinearVelocity(ctx context.Context, extra types.ExtraParams) (r3.Vector, error) {
	m.velocityCount++
	return speed, nil
}

func (m *mock) AngularVelocity(ctx context.Context, extra types.ExtraParams) (spatialmath.AngularVelocity, error) {
	return ang, nil
}

func (m *mock) Orientation(ctx context.Context, extra types.ExtraParams) (spatialmath.Orientation, error) {
	return orie, nil
}

func (m *mock) CompassHeading(ctx context.Context, extra types.ExtraParams) (float64, error) {
	return compass, nil
}
