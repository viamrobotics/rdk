...
func (m *merged) Reconfigure(ctx context.Context, deps resource.Dependencies, conf resource.Config) error {
	...
	firstGoodSensorWithProperties := func(
		...
		for _, name := range names {
			...
			if err != nil {
				logger.CDebugf(ctx, "error getting sensor %v from dependencies", ms.Name().ShortName())
				continue
			}

			props, err := ms.Properties(ctx, nil)
			if err != nil {
				logger.CDebugf(ctx, "error in getting sensor %v properties", ms.Name().ShortName())
				continue
			}
