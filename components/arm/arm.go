...
type Arm interface {
	...
	AllInputs(ctx context.Context, goals [][]referenceframe.Input) error
...
func (r *reconfigurableArm) AllInputs(ctx context.Context, goals [][]referenceframe.Input) error {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.actual.AllInputs(ctx, goals)
}
...
func Move(ctx context.Context, r robot.Robot, a Arm, dst spatialmath.Pose, worldState *referenceframe.WorldState) error {
	...
	return AllWaypoints(ctx, a, solution)
}
...
func Plan(
	...
	extra := map[string]interface{}{"timeout": 20.0}
	...
	if fs.Frame(armName) == nil {
		...
		return motionplan.PlanFrameMotion(ctx, r.Logger(), dst, armFrame, armFrame.InputFromProtobuf(jp), defaultArmPlannerOptions, extra)
	}
	solutionMap, err := motionplan.PlanRobotMotion(ctx, destination, a.ModelFrame(), r, fs, worldState, defaultArmPlannerOptions, extra)
	if err != nil {
		return nil, err
	}
...
// AllWaypoints will visit in turn each of the joint position waypoints generated by a motion planner.
func AllWaypoints(ctx context.Context, a Arm, waypoints [][]referenceframe.Input) error {
	return a.AllInputs(ctx, waypoints)
}
