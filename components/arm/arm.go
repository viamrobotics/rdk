...
func Plan(ctx context.Context, logger golog.Logger, a Arm, dst spatialmath.Pose) ([][]referenceframe.Input, error) {
	...
	planRequest := motionplan.PlanRequest{
		...
		Frame:              model,
		FrameSystem:        nil,
		StartConfiguration: map[string][]referenceframe.Input{model.Name(): model.InputFromProtobuf(jp)},
		ConstraintSpecs:    defaultArmPlannerOptions,
	}
	plan, err := motionplan.PlanMotion(ctx, &planRequest)
	if err != nil {
		return nil, err
	}
	return plan.GetFrameSteps(model.Name())
}
