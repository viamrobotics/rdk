...
func CreateStatus(ctx context.Context, a Arm) (*pb.Status, error) {
	jointPositions, err := a.CurrentInputs(ctx)
	if err != nil {
		return nil, err
	}
	model := a.ModelFrame()
	...
	return &pb.Status{EndPosition: endPosition, JointPositions: a.ModelFrame().ProtobufFromInput(jointPositions), IsMoving: isMoving}, nil
}
