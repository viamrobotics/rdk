...
func accurateSleep(ctx context.Context, duration time.Duration) bool {
	...
	// to, which can be hundreds of microseconsd later (because the process scheduler in the OS only
	// schedules things every millisecond or twos). For use cases like a web server responding to a
	...
	// Unexpectedly, setting the maxBusyWaitTime to less than a millisecond tended to make both
	// approaches have similar errors.
	startTime := time.Now()
	maxBusyWaitTime := 1 * time.Millisecond
