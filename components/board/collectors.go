...
func newAnalogCollector(resource interface{}, params data.CollectorParams) (data.Collector, error) {
	...
	cFunc := data.CaptureFunc(func(ctx context.Context, arg map[string]*anypb.Any) (data.CaptureResult, error) {
		...
		if !ok {
			return res, errtrace.Wrap(data.NewFailedToReadError(params.ComponentName, analogs.String(),
		...
		if err != nil {
			return res, errtrace.Wrap(data.NewFailedToReadError(params.ComponentName, analogs.String(), errors.Wrap(err, "failed to get reader name")))
		}
		...
		if reader, err := board.AnalogByName(analogReaderName); err == nil {
			...
			if err != nil {
				...
				return res, errtrace.Wrap(data.NewFailedToReadError(params.ComponentName, analogs.String(), err))
			}
		}

		ts := data.Timestamps{TimeRequested: timeRequested, TimeReceived: time.Now()}
		return errtrace.Wrap2(data.NewTabularCaptureResult(ts, pb.ReadAnalogReaderResponse{
...
func newGPIOCollector(resource interface{}, params data.CollectorParams) (data.Collector, error) {
	...
	cFunc := data.CaptureFunc(func(ctx context.Context, arg map[string]*anypb.Any) (data.CaptureResult, error) {
		...
		if !ok {
			return res, errtrace.Wrap(data.NewFailedToReadError(params.ComponentName, gpios.String(),
				errors.New("Must supply pin_name in additional params for gpio collector")))
		}

		pinName, err := unmarshalName(pinNameMarshaled)
		if err != nil {
			return res, errtrace.Wrap(data.NewFailedToReadError(
		...
		if gpio, err := board.GPIOPinByName(pinName); err == nil {
			...
			if err != nil {
				...
				return res, errtrace.Wrap(data.NewFailedToReadError(params.ComponentName, gpios.String(), err))
			}
		}
		ts := data.Timestamps{TimeRequested: timeRequested, TimeReceived: time.Now()}
		return errtrace.Wrap2(data.NewTabularCaptureResult(ts, pb.GetGPIOResponse{
			High: value,
		}))
	})
	return errtrace.Wrap2(data.NewCollector(cFunc, params))
...
func unmarshalName(nameMarshaled *anypb.Any) (string, error) {
	...
	if err := nameMarshaled.UnmarshalTo(&structVal); err != nil {
		return "", errtrace.Wrap(errors.Wrap(err, "failed to unmarshal name"))
	}
	...
	default:
		return "", errtrace.Wrap(errors.New("name must be a string"))
	}
}
