...
func TestClient(t *testing.T) {
	...
	workingEncoder.ResetPositionFunc = func(ctx context.Context, offset float64, extra map[string]interface{}) error {
		actualExtra = extra
		return nil
	}
	workingEncoder.GetPositionFunc = func(ctx context.Context, extra map[string]interface{}) (float64, error) {
		actualExtra = extra
		return 42.0, nil
	}

	failingEncoder.ResetPositionFunc = func(ctx context.Context, offset float64, extra map[string]interface{}) error {
		return errors.New("set to zero failed")
	}
	failingEncoder.GetPositionFunc = func(ctx context.Context, extra map[string]interface{}) (float64, error) {
	...
	t.Run("client tests for failing encoder", func(t *testing.T) {
		err = failingEncoderClient.ResetPosition(context.Background(), 0.5, nil)
		test.That(t, err, test.ShouldNotBeNil)

		pos, err := failingEncoderClient.GetPosition(context.Background(), nil)
		test.That(t, err, test.ShouldNotBeNil)
	...
	t.Run("dialed client tests for working encoder", func(t *testing.T) {
		...
		pos, err := workingEncoderDialedClient.GetPosition(context.Background(), nil)
		test.That(t, err, test.ShouldBeNil)
		test.That(t, pos, test.ShouldEqual, 42.0)

		err = workingEncoderDialedClient.ResetPosition(context.Background(), 0.5, nil)
		test.That(t, err, test.ShouldBeNil)
