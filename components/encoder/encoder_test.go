...
func TestReconfigurableEncoder(t *testing.T) {
	actualEncoder1 := &mockLocal{Name: testEncoderName}
	reconfEncoder1, err := encoder.WrapWithReconfigurable(actualEncoder1, resource.Name{})
	test.That(t, err, test.ShouldBeNil)

	actualEncoder2 := &mockLocal{Name: testEncoderName2}
	...
	test.That(t, actualEncoder1.reconfCount, test.ShouldEqual, 2)

	test.That(t, actualEncoder1.posCount, test.ShouldEqual, 0)
	test.That(t, actualEncoder2.posCount, test.ShouldEqual, 0)
	result, err := reconfEncoder1.(encoder.Encoder).GetPosition(context.Background(), nil)
	test.That(t, err, test.ShouldBeNil)
	test.That(t, result, test.ShouldResemble, position)
	test.That(t, actualEncoder1.posCount, test.ShouldEqual, 0)
	...
	err = reconfEncoder1.Reconfigure(context.Background(), reconfEncoder3)
	test.That(t, err, test.ShouldNotBeNil)
	test.That(t, err, test.ShouldBeError, rutils.NewUnexpectedTypeError(reconfEncoder1, reconfEncoder3))
	test.That(t, actualEncoder3.reconfCount, test.ShouldEqual, 0)

	err = reconfEncoder3.Reconfigure(context.Background(), reconfEncoder1)
	test.That(t, err, test.ShouldNotBeNil)
	test.That(t, err, test.ShouldBeError, rutils.NewUnexpectedTypeError(reconfEncoder3, reconfEncoder1))

	actualEncoder4 := &mock{Name: testEncoderName2}
	reconfEncoder4, err := encoder.WrapWithReconfigurable(actualEncoder4, resource.Name{})
	test.That(t, err, test.ShouldBeNil)
	test.That(t, reconfEncoder4, test.ShouldNotBeNil)

	err = reconfEncoder3.Reconfigure(context.Background(), reconfEncoder4)
	test.That(t, err, test.ShouldBeNil)
	test.That(t, reconfEncoder3, test.ShouldResemble, reconfEncoder4)
}
...
func TestResetPosition(t *testing.T) {
	...
	err = reconfEncoder1.(encoder.Encoder).ResetPosition(context.Background(), 0, nil)
	test.That(t, err, test.ShouldBeNil)
...
func TestGetPosition(t *testing.T) {
	...
	test.That(t, actualEncoder1.posCount, test.ShouldEqual, 0)
	pos1, err := reconfEncoder1.(encoder.Encoder).GetPosition(context.Background(), nil)
	test.That(t, err, test.ShouldBeNil)
...
func (m *mock) ResetPosition(ctx context.Context, offset float64, extra map[string]interface{}) error {
	m.zeroCount++
	return nil
}
