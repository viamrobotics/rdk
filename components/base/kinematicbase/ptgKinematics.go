...
func (ptgk *ptgBaseKinematics) ErrorState(ctx context.Context, plan [][]referenceframe.Input, currentNode int) (spatialmath.Pose, error) {
	...
	ptgk.logger.Debugf("curr position %#v", spatialmath.PoseToProtobuf(actualPIF))
	ptgk.logger.Debugf("nominal position %#v", spatialmath.PoseToProtobuf(nominalPose))
	ptgk.logger.Debugf("error state %#v", spatialmath.PoseToProtobuf(spatialmath.PoseBetween(nominalPose, actualPIF)))
