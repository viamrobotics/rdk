...
func FromDependencies(deps resource.Dependencies, name string) (resource.Resource, error) {
	return errtrace.Wrap2(resource.FromDependencies[resource.Resource](deps, Named(name)))
}
...
func FromRobot(r robot.Robot, name string) (resource.Resource, error) {
	return errtrace.Wrap2(robot.ResourceFromRobot[resource.Resource](r, Named(name)))
}
...
func FromProvider(provider resource.Provider, name string) (resource.Resource, error) {
	return errtrace.Wrap2(resource.FromProvider[resource.Resource](provider, Named(name)))
}
