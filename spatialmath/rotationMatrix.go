...
// Rotation Matrix is a 3x3 matrix in row major order.
...
func (rm *RotationMatrix) Quaternion() quat.Number {
	...
	if (rm[0] > rm[4]) && (rm[0] > rm[8]) {
		s := float64(2.0 * math.Sqrt(float64(1.0+rm[0]-rm[4]-rm[8])))
		return quat.Number{(rm[5] - rm[7]) / s, 0.25 * s, (rm[3] + rm[1]) / s, (rm[6] + rm[2]) / s}
	}

	if rm[4] > rm[8] {
		s := float64(2.0 * math.Sqrt(float64(1.0+rm[4]-rm[0]-rm[8])))
		return quat.Number{(rm[6] - rm[2]) / s, (rm[3] + rm[1]) / s, 0.25 * s, (rm[7] + rm[5]) / s}
	}

	s := float64(2.0 * math.Sqrt(float64(1.0+rm[8]-rm[0]-rm[4])))
