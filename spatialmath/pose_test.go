...
func TestBasicPoseConstruction(t *testing.T) {
	...
	test.That(t, p.Orientation().OrientationVectortorRadians(), test.ShouldResemble, &OrientationVector{0, 0, 0, 1})

	// point at +Y, rotate 90 degrees
	ov := &OrientationVector{math.Pi / 2, 0, 1, 0}
	ov.Normalize()

	p = NewPoseFromOrientationVectortor(r3.Vector{1, 2, 3}, ov)
	ovCompare(t, p.Orientation().OrientationVectortorRadians(), ov)
	...
	p1 := NewPoseFromOrientationVectortor(r3.Vector{1, 2, 3}, ov)
	p2 := NewPoseFromPoint(r3.Vector{1, 2, 3})
	pComp := Compose(p1, p2)
	ptCompare(t, pComp.Point(), r3.Vector{0, 5, 5})

	p2 = NewPoseFromOrientationVectortor(r3.Vector{2, 2, 4}, ov)
