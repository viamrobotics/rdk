...
func TestRevoluteFrame(t *testing.T) {
	...
	frame := &rotationalFrame{"test", axis, Limit{Min: -math.Pi / 2, Max: math.Pi / 2}} // limits between -90 and 90 degrees
	// expected output
	...
	_, err = frame.Transform(input)
	test.That(t, err, test.ShouldBeError, errors.Errorf("%.5f input out of rev frame bounds %v", utils.DegToRad(overLimit), frame.DoF()[0]))
	// gets the correct limits back
	limit := frame.DoF()
