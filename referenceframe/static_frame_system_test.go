...
func TestSimpleFrameTranslation(t *testing.T) {
	...
	testTransformPoint(t, fs, FrameConfigurations{}, poseWorld, poseFrame)
	testTransformPoint(t, fs, FrameConfigurations{}, poseFrame, poseWorld)
}
...
func TestFrameTranslation(t *testing.T) {
	...
	testTransformPoint(t, fs, FrameConfigurations{}, poseStart, poseEnd)
	testTransformPoint(t, fs, FrameConfigurations{}, poseEnd, poseStart)
}
...
func TestGeomtriesTransform(t *testing.T) {
	...
	test.That(t, err, test.ShouldBeNil)
	tf, err := fs.Transform(FrameConfigurations{}, geometries, "frame2")
	test.That(t, err, test.ShouldBeNil)
	...
	tf, err = fs.Transform(FrameConfigurations{}, gf, "frame3")
	test.That(t, err, test.ShouldBeNil)
	framedGeometries, _ = tf.(*GeometriesInFrame)
...
func TestSystemSplitAndRejoin(t *testing.T) {
	...
	_, err = fs.Transform(FrameConfigurations{}, NewPoseInFrame("frame4", spatial.NewPoseFromPoint(r3.Vector{2, 0, 0})), "frame2")
...
func testTransformPoint(t *testing.T, fs FrameSystem, positions FrameConfigurations, start, end *PoseInFrame) {
	t.Helper()
