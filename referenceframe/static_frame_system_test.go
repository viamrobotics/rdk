...
func TestSimpleFrameTranslation(t *testing.T) {
	...
	testTransformPoint(t, fs, FrameConfgurations{}, poseWorld, poseFrame)
	testTransformPoint(t, fs, FrameConfgurations{}, poseFrame, poseWorld)
}
...
func TestFrameTranslation(t *testing.T) {
	...
	testTransformPoint(t, fs, FrameConfgurations{}, poseStart, poseEnd)
	testTransformPoint(t, fs, FrameConfgurations{}, poseEnd, poseStart)
}
...
func TestGeomtriesTransform(t *testing.T) {
	...
	test.That(t, err, test.ShouldBeNil)
	tf, err := fs.Transform(FrameConfgurations{}, geometries, "frame2")
	test.That(t, err, test.ShouldBeNil)
	...
	tf, err = fs.Transform(FrameConfgurations{}, gf, "frame3")
	test.That(t, err, test.ShouldBeNil)
	framedGeometries, _ = tf.(*GeometriesInFrame)
...
func TestSystemSplitAndRejoin(t *testing.T) {
	...
	_, err = fs.Transform(FrameConfgurations{}, NewPoseInFrame("frame4", spatial.NewPoseFromPoint(r3.Vector{2, 0, 0})), "frame2")
...
func testTransformPoint(t *testing.T, fs FrameSystem, positions FrameConfgurations, start, end *PoseInFrame) {
	t.Helper()
