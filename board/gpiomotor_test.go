...
func TestMotorDirPWM(t *testing.T) {
	...
	m, err := NewGPIOMotor(b, MotorConfig{Pins: map[string]string{"dir": "1", "en": "2", "pwm": "3"}, PWMFreq: 4000}, logger)
	test.That(t, err, test.ShouldBeNil)
...
func TestMotorAB(t *testing.T) {
	...
	m, err := NewGPIOMotor(b, MotorConfig{Pins: map[string]string{"a": "1", "b": "2", "en": "3"}, PWMFreq: 4000}, logger)
	test.That(t, err, test.ShouldBeNil)
