...
func TestFailedModuleTracking(t *testing.T) {
	os.Setenv("VIAM_TCP_SOCKETS", "yes")
	t.Cleanup(func() { os.Unsetenv("VIAM_TCP_SOCKETS") })
	...
	// Add a working module
	workingMod := []config.Module{
		{
			Name:    "working-module",
			ExePath: rtestutils.BuildTempModule(t, "module/testmodule"),
			Type:    config.ModuleTypeLocal,
	...
	err := mgr.Add(ctx, workingMod...)
	test.That(t, err, test.ShouldBeNil)

	// Add two failing modules (invalid exe paths)
	failingMod1 := []config.Module{
		{
	...
	err = mgr.Add(ctx, failingMod1...)
	test.That(t, err, test.ShouldNotBeNil)

	failingMod2 := []config.Module{
		{
			Name:    "failing-module-2",
			ExePath: "/nonexistent/path/to/module2",
			Type:    config.ModuleTypeLocal,
		},
	}
	...
	t.Run("GetFailedModules returns failing modules properly", func(t *testing.T) {
		...
		test.That(t, len(failedModules), test.ShouldEqual, 2)
	})
	...
	t.Run("Module that starts failing gets added to failedModules", func(t *testing.T) {
		...
		invalidReconfig := config.Module{
			Name:    "working-module",
			ExePath: "/nonexistent/path/to/invalid",
			Type:    config.ModuleTypeLocal,
		}

		_, err := mgr.Reconfigure(ctx, invalidReconfig)
		// Reconfigure should fail because the module can't start with invalid path
		test.That(t, err, test.ShouldNotBeNil)
