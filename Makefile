...
deb-server: server cameras
	...
	cp -r etc/packaging/viam-server-0.3/ etc/packaging/work/
	install -D $(BIN_OUTPUT_PATH)/server etc/packaging/work/viam-server-0.3/usr/bin/viam-server
	install -D etc/camera_servers/intelrealserver etc/packaging/work/viam-server-0.3/usr/bin/intelrealserver
	install -D etc/camera_servers/royaleserver etc/packaging/work/viam-server-0.3/usr/bin/royaleserver
	install -m 644 -D web/runtime-shared/templates/* --target-directory=etc/packaging/work/viam-server-0.3/usr/share/viam/templates/
	install -m 644 -D web/runtime-shared/static/main.js etc/packaging/work/viam-server-0.3/usr/share/viam/static/main.js
	install -m 644 -D web/runtime-shared/static/third-party/* --target-directory=etc/packaging/work/viam-server-0.3/usr/share/viam/static/third-party
	cd etc/packaging/work/viam-server-0.3/ \
	&& dch -v 0.3+`date -u '+%Y%m%d%H%M'` "Auto-build from commit `git log --pretty=format:'%h' -n 1`" \
	&& dch -r viam \
	&& dpkg-buildpackage -us -uc -b \

deb-install: deb-server
	sudo dpkg -i etc/packaging/work/viam-server_0.3+*.deb
